...
<body class="no-skin">
								...
								<table id="simple-table"
									...
									<thead>
										<tr>
											...
											<th class="center">店铺显示顺序</th>
											<th class="center">店铺照片</th>
											<th class="center">店铺地址</th>
											<th class="center">店铺营业时间</th>
											<th class="center">微信号</th>
											<th class="center">微信二维码</th>
											<th class="center">支付宝二维码</th>
											<th class="center">店主人名字</th>
												...
												<c:if test="${QX.cha == 1 }">
													<c:forEach items="${varList}" var="var" varStatus="vs">
														<tr>
															...
															<td class='center'>
																<a href="javascript:goGoods('${var.ID }')">${var.NAME}</a>
															</td>
															<td class='center'>${var.INTEGRAL}</td>
															<td class='center'>${var.DISPLAY_ORDER}</td>
															<td class='center'>
																<c:if test="${var.PATH != null && var.PATH != ''}">
																	<img src="<%=basePath%>uploadFiles/uploadImgs/${var.PATH}" alt="${var.NAME}" width="100">
								  								</c:if>
															</td>
															<td class='center'>${var.ADDRESS}</td>
															<td class='center'>${var.BUSSTIME_FROM} - ${var.BUSSTIME_TO}</td>
															<td class='center'>${var.WECHAT_ID}</td>
															<td class='center'>
																<c:if test="${var.WECHAT_QRCODE != null && var.WECHAT_QRCODE != ''}">
																	<img src="<%=basePath%>uploadFiles/uploadImgs/${var.WECHAT_QRCODE}" alt="${var.NAME}" width="100">
																</c:if>
															</td>
															<td class='center'>
																<c:if test="${var.ALIPAY_QRCODE != null && var.ALIPAY_QRCODE != ''}">
																	<img src="<%=basePath%>uploadFiles/uploadImgs/${var.ALIPAY_QRCODE}" alt="${var.NAME}" width="100">
																</c:if>
															</td>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=false;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>store/goAdd.do?flag='+'store';
			 diag.Width = 700;
			 diag.Height = 700;
		...
		function edit(Id){
			 ...
			 diag.Drag=false;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>store/goEdit.do?ID='+Id+'&flag='+'store';
			 diag.Width = 700;
			 diag.Height = 700;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		function goGoods(ID){
			top.jzts();
			window.location.href="<%=basePath%>goods/list.do?STORE_ID="+ID;
		};
	</script>
