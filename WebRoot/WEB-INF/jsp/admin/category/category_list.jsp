								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>
												<img id="imgAD" src="<%=basePath%>uploadFiles/uploadImgs/${var.ICON}" alt="" width="100"/> 
					  							<input type="hidden" name="ICON" id="ICON" value="${var.ICON}" /><!-- 图片成功上传后的url -->
											</td>
	...
	<script type="text/javascript">
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>category/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=false;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>category/goEdit.do?ID='+Id;
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
