...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="beautyorder/list.do" method="post" name="Form" id="Form">
						...
						<label class="col-sm-3 control-label no-padding-right" for="form-field-1" style="width: 100%;text-align:left;font-weight:bolder">总利润：${countPd.PROFIT!=undefined ? countPd.PROFIT : 0}元；总销售额：${countPd.SALE_COUNT!=undefined ? countPd.SALE_COUNT : 0}元；总成本：${countPd.ALL_COST!=undefined ? countPd.ALL_COST : 0}元（快递费：${countPd.TAKYUBIN_FEE!=undefined ? countPd.TAKYUBIN_FEE : 0}元；成本费：${countPd.COST!=undefined ? countPd.COST : 0}元）</label>
						<!-- 检索  -->
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									...
									<a class="btn btn-mini btn-success" onclick="resetPwd();">重置密码</a>
									</c:if>
								</td>
	...
	<script type="text/javascript">
		...
		function addOrder(ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="添加订单";
			 diag.URL = '<%=basePath%>beautyorder/goAdd.do?ID='+ID;
			 diag.Width = 800;
			 diag.Height = 500;
		...
		function resetPwd() {
			$.ajax({
				type: "GET",
				url: '<%=basePath%>beautyorder/reset.do',
				dataType:'json',
				cache: false,
		...
		function info(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改订单";
			 diag.URL = '<%=basePath%>beautyorder/goInfo.do?ID='+id+'&flag=0';
			 diag.Width = 800;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
				if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function edit(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改订单";
			 diag.URL = '<%=basePath%>beautyorder/goEdit.do?ID='+id;
			 diag.Width = 800;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
				if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
