		...
		<div class="main-content-inner">
			<div class="page-content">
				<div class="row">
					...
					<form action="carehistory/${msg }_order.do" name="Form" id="Form" method="post">
						...
						<table id="table_report" class="table table-striped table-bordered table-hover">
							...
							<tr>
								<td style="text-align: center;" colspan="10">
									...
									<c:if test="${QX.add == 1 }">
									<a class="btn btn-mini btn-primary" onclick="add('${pd.ID}');">确认消费</a>
									</c:if>
...
<!-- /.main-container -->
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
		...
		function save(){
			...
			top.jzts();
			var url = "<%=basePath%>carehistory/save_order.do?tm="+new Date().getTime();
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>carehistory/delete_order.do?ID="+Id+"&tm="+new Date().getTime();
					$("#Form").attr("action",url);
		...
		//消费
		function add(Id){
			bootbox.confirm("确定已经消费吗？", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>carehistory/addtohis.do?ID="+Id+"&tm="+new Date().getTime();
					$("#Form").attr("action",url);
		...
		$(function() {
			...
			$('#PRICE_ORI').prop("disabled",false);
			$('#PRICE_DISCUT').prop("disabled",false);
			$('#SETTLE_TYPE').prop("disabled",false);
			//时间框
